...
class OP_Node(CopperObject, OP_Parameters):
    ...
    def cook(self, force:bool=False, frame_range=()):
    ...
    def input(self, input_index:int):
        try:
    ...
    def setInput(self, input_index:int, node, output_index=0):
    ...
    def inputDataSocket(self, index:int):
        return self._input_sockets[index]

    def outputDataSocket(self, index:int):
    ...
    def __eq__(self, node):
        ...
        '''
        assert isinstance(node, OP_Node), "node is not OP_Node instance"
        if not node:
            return False
