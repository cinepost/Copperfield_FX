...
__kernel void filter(
        ...
        int img_width, int img_height, float density, int quality, int mode) {
	...
	for( int a=0; a < quality; a++ ){
		for( int b=0; b < quality; b++ ){
			...
			float jitter_x = 10.0f * (noise3D(x,y,a * 1.234f) - 0.5f) / (float)img_width; // z used as seed
			float jitter_y = 10.0f * (noise3D(x,y,b * 2.468f) - 0.5f) / (float)img_height;
			
			// supersampled, jittered, normalized float image coordinate
			float2 coord = (float2)(
				((float)x + (float)a / (float)quality) / (float)img_width, 
				((float)y + (float)b / (float)quality) / (float)img_height
			);
			...
			acc += res;
			//acc += (float3)(coord.x , coord.y , 0.0f);
		}	
	}
