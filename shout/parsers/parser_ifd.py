...
class ParserIFD(ParserBase):
	...
	def grammar(self):
		...
		if ifd_grammar is None:
			...
			ray_detail_filename = quotedString | Word(printables)
			...
			ray_declare_value = quotedString | number | Word(alphanums)
			...
			ray_property_value = Group(OneOrMore(integer | floatnum | Word(alphanums + "_") | quotedString))
			...
			set_env = Keyword('setenv') + Group(Word(alphanums) + Suppress(Literal('=')) + quotedString)

			otprefer = Keyword('otprefer') + quotedString + quotedString

			ifd_grammar = (set_env | ray_time | ray_version | ray_start | ray_end | ray_image | ray_quit | ray_transform | ray_reset 
				| ray_property | ray_procedural | ray_declare | ray_detail_1 | otprefer | Word(printables))# + StringEnd()
